...
function PaiementSimulationContent() {
  ...
  const handleSuccess = async () => {
    setLoading(true);
    try {
      await api.post(`/api/abonnement/webhook`, {
        abonnementId,
        transactionId,
        status: "SUCCESS",
      });
  ...
  const handleFail = async () => {
    setLoading(true);
    try {
      await api.post(`/api/paiement/webhook/fail`, {
        abonnementId,
        transactionId,
        status: "FAILED",
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-green-50 p-6">
      <div className="bg-white shadow-2xl rounded-3xl p-8 w-full max-w-md text-center">
        ...
        <p className="text-gray-500 mb-6">Transaction : {transactionId}</p>
        <p className="text-3xl font-bold mb-8">{montant} FCFA</p>

        {status === "WAITING" && (
          <div className="mb-6">
            <Loader2 className="animate-spin mx-auto text-[#063c28]" size={40} />
            <p className="mt-2 text-gray-600">En attente de paiement...</p>
          </div>
        )}
        ...
        {status === "SUCCESS" && (
          <div className="mb-6 text-green-600">
            ...
            <p className="mt-2 font-semibold">Paiement réussi ✔</p>
          </div>
        )}
        ...
        {status === "FAILED" && (
          <div className="mb-6 text-red-500">
            ...
            <p className="mt-2 font-semibold">Paiement échoué ❌</p>
          </div>
        )}
